...
from sherpa.data_collection import MongoDBBackend, MongoDBClient
try:
...
def test_database(test_dir):
    ...
    with MongoDBBackend(test_dir, port=db_port) as db:
        ...
        db2 = MongoDBBackend(test_dir, port=db_port)
...
def test_database_args(test_dir):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        with MongoDBBackend(test_dir, port=db_port,
                                       mongodb_args={
            ...
            db2 = MongoDBBackend(test_dir, port=custom_port)
            with pytest.raises(OSError):
                db2.start()


def test_client_test_mode_send_metrics_does_nothing():
    client = MongoDBClient(test_mode=True)
