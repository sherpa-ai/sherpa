...
class MainLoop():
    ...
    """
    def __init__(self, fname, algorithm, dir='./', loss='loss', results_table=None, environment=None, submit_options=''):
        ...
        self.results_table = results_table if results_table is not None else ResultsTable(self.dir, recreate=False)
        return
    ...
    def _start_subprocess(self, queue, rval):
        ...
        if self.job_management == 'local':
            p = Process(target=self._subprocess_local, args=(queue, run_id, modelfile, historyfile, hparams, epochs))
        elif self.job_management == 'sge':
            p = Process(target=self._subprocess_sge, args=(queue, run_id, modelfile, historyfile, hparams, epochs))
        else:
            raise Exception('Bad value of job_management: %s' % self.job_management)
        p.start()
        return p
    
    def _subprocess_local(self, queue, run_id, modelfile, historyfile, hparams, epochs):
        ...
        queue.put([run_id, hparams, rval, historyfile]) # See _read_queue for details.
    ...
    def _subprocess_sge(self, queue, run_id, modelfile, historyfile, hparams, epochs):
        ...
        python_script = 'import %s as mymodule\n' % self.fname.rsplit('.', 1)[0] # Module.
        python_script += 'rval=mymodule.main(\'%s\', \'%s\', hparams=%s, epochs=%d, verbose=2)' % (modelfile, historyfile, hparams, epochs)
        ...
        submit_command = 'qsub -S /bin/bash -wd %s -j y -o %s -e %s %s' % (os.getcwd(), sgeoutfile, sgeoutfile, self.submit_options)
